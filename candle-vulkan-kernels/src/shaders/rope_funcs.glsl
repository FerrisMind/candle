...
void rope_norm(const uint i0, const uint i1, const uint i2, const uint i3, rope_params p) {
    ...
    if (i0 >= p.n_dims) {
        rope_data_d[idst + 0] = rope_data_a[ix + 0];
        rope_data_d[idst + 1] = rope_data_a[ix + 1];
    ...
    const float x0 = ROPE_STORAGE_TO_FLOAT(rope_data_a[ix + 0]);
    const float x1 = ROPE_STORAGE_TO_FLOAT(rope_data_a[ix + 1]);

    rope_data_d[idst + 0] = ROPE_STORAGE_FROM_FLOAT(x0*cos_theta - x1*sin_theta);
    rope_data_d[idst + 1] = ROPE_STORAGE_FROM_FLOAT(x0*sin_theta + x1*cos_theta);
}
...
void rope_neox(const uint i0, const uint i1, const uint i2, const uint i3, rope_params p) {
    ...
    if (i0 >= p.n_dims) {
        rope_data_d[idst + i0/2 + 0] = rope_data_a[ix + i0/2 + 0];
        rope_data_d[idst + i0/2 + 1] = rope_data_a[ix + i0/2 + 1];
    ...
    const float x0 = ROPE_STORAGE_TO_FLOAT(rope_data_a[ix + 0]);
    const float x1 = ROPE_STORAGE_TO_FLOAT(rope_data_a[ix + p.n_dims/2]);

    rope_data_d[idst + 0]          = ROPE_STORAGE_FROM_FLOAT(x0*cos_theta - x1*sin_theta);
    rope_data_d[idst + p.n_dims/2] = ROPE_STORAGE_FROM_FLOAT(x0*sin_theta + x1*cos_theta);
}
...
void rope_vision(const uint i0, const uint i1, const uint i2, const uint i3, rope_params p) {
    ...
    const float x0 = ROPE_STORAGE_TO_FLOAT(rope_data_a[ix + 0]);
    const float x1 = ROPE_STORAGE_TO_FLOAT(rope_data_a[ix + p.n_dims]);

    rope_data_d[idst + 0]        = ROPE_STORAGE_FROM_FLOAT(x0*cos_theta - x1*sin_theta);
    rope_data_d[idst + p.n_dims] = ROPE_STORAGE_FROM_FLOAT(x0*sin_theta + x1*cos_theta);
}
