...
impl C2f {
    fn load(vb: VarBuilder, c1: usize, c2: usize, n: usize, shortcut: bool) -> Result<Self> {
        ...
        for idx in 0..n {
            bottleneck.push(Bottleneck::load(
                vb.pp(format!("bottleneck.{idx}")),
                c,
                c,
                shortcut,
            )?);
...
fn model_path() -> String {
    std::env::var("CANDLE_YOLO_MODEL")
        .unwrap_or_else(|_| r"G:\models\candle-yolo-v8\yolov8s.safetensors".to_string())
}
...
fn read_real_input() -> Result<(Vec<f32>, Vec<usize>)> {
    let path = std::env::var("CANDLE_YOLO_IMG_BIN")
...
fn run_real_model_bisect(cpu: &Device, wgpu: &Device) -> Result<()> {
    ...
    let wgpu_vb =
        unsafe { VarBuilder::from_mmaped_safetensors(&[model_path()], DType::F32, wgpu) }?;
    ...
    let stage_labels = [
        "net.x2", "net.x3", "net.x5", "head1", "head2", "head3", "pred",
    ];
    let cpu_stages = [
        downcast(&s_cpu.net1)?,
        downcast(&s_cpu.net2)?,
    ...
    for i in 0..stage_labels.len() {
        ...
        eprintln!(
            "[bisect] stage {:8} cpu-vs-wgpu relerr={r:.6} maxabs={ma:.6} (n={})",
            stage_labels[i],
            cpu_stages[i].len()
        );
    }
    ...
    for i in 0..fine_labels.len() {
        ...
        eprintln!(
            "[bisect] fine {:10} cpu-vs-wgpu relerr={r:.6} maxabs={ma:.6} (n={})",
            fine_labels[i],
            fine_cpu[i].len()
        );
    }
    ...
    assert!(
        r < 5e-3,
        "yolo real-model pred cpu-vs-wgpu relerr={r:.5} >= 5e-3 (bug present)"
    );
    Ok(())
}
...
fn avgpool_kernel2_isolated() -> Result<()> {
    ...
    for (name, shape) in [
        ...
        assert!(
            ma < 1e-3 && r < 1e-3,
            "[{name}] avg_pool2d(2) wgpu-vs-cpu relerr={r:.5} maxabs={ma:.5}"
        );
    }
    Ok(())
}
...
fn vae_conv_out_green() -> Result<()> {
    ...
    let in_cpu =
        Tensor::from_vec(data.clone(), [1, 512, 32, 32].as_slice(), &cpu)?.to_dtype(DType::F32)?;
    let in_wgpu =
        Tensor::from_vec(data, [1, 512, 32, 32].as_slice(), &wgpu)?.to_dtype(DType::F32)?;
...
fn maxpool_isolated_shapes() -> Result<()> {
    ...
    for (name, shape) in [
        ...
        assert!(
            ma < 1e-3 && r < 1e-3,
            "[{name}] max_pool2d wgpu-vs-cpu relerr={r:.5} maxabs={ma:.5} (pool corruption)"
        );
    }
    Ok(())
}
