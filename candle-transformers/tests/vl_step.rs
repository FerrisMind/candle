...
fn step() -> Result<()> {
    ...
    let v_v = t_v.i(2)?.squeeze(0)?.contiguous()?.to_dtype(DType::F32)?;
    vk.synchronize()?;
    println!("3 q {}", md(&q_v, &q_c)?);
    println!("3 k {}", md(&k_v, &k_c)?);
    println!("3 v {}", md(&v_v, &v_c)?);
    let (qr_c, kr_c) = apply_rope(&q_c, &k_c, &cos, &sin)?;
    let (qr_v, kr_v) = apply_rope(&q_v, &k_v, &cos.to_device(&vk)?, &sin.to_device(&vk)?)?;
    vk.synchronize()?;
    ...
    let ag = (qg.matmul(&kg.transpose(2, 3)?.contiguous()?)? / 8.0)?;
    vk.synchronize()?;
    println!("5 scores {}", md(&ag, &ac)?);
    let ac = candle_nn::ops::softmax_last_dim(&ac)?;
    let ag = candle_nn::ops::softmax_last_dim(&ag)?;
    vk.synchronize()?;
    println!("6 soft {}", md(&ag, &ac)?);
    let oc = ac.matmul(&vc.contiguous()?)?;
    let og = ag.matmul(&vg.contiguous()?)?;
    vk.synchronize()?;
    ...
    let oc = proj_c.forward(&oc)?;
    let og = proj_v.forward(&og)?;
    vk.synchronize()?;
