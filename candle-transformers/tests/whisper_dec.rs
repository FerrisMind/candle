...
fn stages() -> Result<()> {
    ...
    let config: whisper::Config =
        serde_json::from_str(&std::fs::read_to_string(&config_p)?).unwrap();
    let cpu = Device::Cpu;
    let vk = Device::new_vulkan(0)?;
    let load = |dev: &Device| -> Result<whisper::model::Whisper> {
        let vb = unsafe { VarBuilder::from_mmaped_safetensors(&[&weights], whisper::DTYPE, dev)? };
        whisper::model::Whisper::load(&vb, config.clone())
    };
    let mut m_c = load(&cpu)?;
    let mut m_v = load(&vk)?;
    let mel_len = config.num_mel_bins * whisper::N_FRAMES;
    let mel: Vec<f32> = (0..mel_len)
        .map(|i| {
            let mixed = (i as u64)
                .wrapping_mul(6364136223846793005)
                .wrapping_add(0x5151u64.wrapping_mul(1442695040888963407));
    ...
    // Access encoder fields - they're pub(crate) maybe not. Use full forward only.
    // Compare with flush between by running on CPU then stepwise via public API only.
    // Use encoder.forward which is all we have.

    // Check if positional embedding is issue - compare after full
    let e_c = m_c.encoder.forward(&mel_c, true)?;
    let e_v = m_v.encoder.forward(&mel_v, true)?;
    vk.synchronize()?;
    println!("encoder {}", md(&e_c, &e_v)?);

    // decoder part with CPU encoder states on both
    let ids = Tensor::from_slice(&[1u32], (1, 1), &cpu)?;
    let ids_v = Tensor::from_slice(&[1u32], (1, 1), &vk)?;
    let e_v_from_cpu = e_c.to_device(&vk)?;
    let h_c = m_c.decoder.forward(&ids, &e_c, true)?;
    let h_v = m_v.decoder.forward(&ids_v, &e_v_from_cpu, true)?;
    vk.synchronize()?;
    println!("decoder w/ cpu encoder state {}", md(&h_c, &h_v)?);
    let l_c = m_c.decoder.final_linear(&h_c)?;
    let l_v = m_v.decoder.final_linear(&h_v)?;
    println!("logits w/ cpu encoder {}", md(&l_c, &l_v)?);
    Ok(())
}
