...
fn expand_cat_mask() -> Result<()> {
    ...
    let q = Tensor::randn(0f32, 1.0, (1, 18, 6, 32), &cpu)?
        .transpose(1, 2)?
        .contiguous()?;
    let k = q.clone();
    let v = Tensor::randn(0f32, 1.0, (1, 18, 6, 32), &cpu)?
        .transpose(1, 2)?
        .contiguous()?;
    let qg = q.to_device(&wg)?;
    let kg = k.to_device(&wg)?;
    let vg = v.to_device(&wg)?;
