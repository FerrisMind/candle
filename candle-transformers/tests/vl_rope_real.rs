...
fn rope_real() -> Result<()> {
    ...
    let _k = qkv_t.i(1)?.squeeze(0)?.contiguous()?.to_dtype(DType::F32)?;
    ...
    let cos_uv = cos
        .to_device(&vk)?
        .contiguous()?
        .unsqueeze(candle::D::Minus2)?;
    println!(
        "broadcast mul cos {}",
        maxdiff(
            &q.to_device(&vk)?.broadcast_mul(&cos_uv)?,
            &q.broadcast_mul(&cos_u)?
        )?
    );
    println!(
        "rotate_half {}",
        maxdiff(&rotate_half(&q.to_device(&vk)?)?, &rotate_half(&q)?)?
    );
    Ok(())
}
