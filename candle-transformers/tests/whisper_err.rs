...
fn whisper_error_stats() -> Result<()> {
    let _config_path = std::env::var("USERPROFILE").unwrap() + "\\.cache\\huggingface\\hub";
    ...
    let dev_vb =
        unsafe { VarBuilder::from_mmaped_safetensors(&[&weights], whisper::DTYPE, &device)? };
    ...
    println!(
        "n={} max_abs={} max_rel={} rmse={} rmse_rel={}",
        ed.len(),
        max_abs,
        max_rel,
        (sum_sq / ed.len() as f32).sqrt(),
        (sum_sq / sum_e.max(1e-12)).sqrt()
    );
