...
fn report(label: &str, a: &Tensor, b: &Tensor) -> Result<()> {
    println!("{label:24} md={:.4e}", maxdiff(a, b)?);
    Ok(())
}
...
fn after_attn() -> Result<()> {
    ...
    let q_g = rope_in(
        &q_g.reshape((1, seq_len, cfg.n_heads, head_dim))?,
        &cos_g,
        &sin_g,
    )?;
    let k_g = rope_in(
        &k_g.reshape((1, seq_len, cfg.n_heads, head_dim))?,
        &cos_g,
        &sin_g,
    )?;
    ...
    let g_g2 = candle_nn::linear_no_bias(
        cfg.dim,
        cfg.hidden_dim,
        vb_g.pp("model.layers.0.mlp.gate_proj"),
    )?
    ...
    let u_g2 = candle_nn::linear_no_bias(
        cfg.dim,
        cfg.hidden_dim,
        vb_g.pp("model.layers.0.mlp.up_proj"),
    )?
    ...
    let d_g2 = candle_nn::linear_no_bias(
        cfg.hidden_dim,
        cfg.dim,
        vb_g.pp("model.layers.0.mlp.down_proj"),
    )?
