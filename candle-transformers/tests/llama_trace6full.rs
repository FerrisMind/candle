...
fn t6full() -> Result<()> {
    ...
    for layer in 0..cfg.n_layers {
        ...
        let rms1_c = candle_nn::rms_norm(
            cfg.dim,
            cfg.norm_eps,
            vb_c.pp(format!("model.layers.{layer}.input_layernorm")),
        )?;
        let rms1_g = candle_nn::rms_norm(
            cfg.dim,
            cfg.norm_eps,
            vb_g.pp(format!("model.layers.{layer}.input_layernorm")),
        )?;
        let n_c = rms1_c.forward(&x_c)?;
        let n_g = rms1_g.forward(&x_g)?;
        let q_c = candle_nn::linear_no_bias(
            cfg.dim,
            cfg.dim,
            vb_c.pp(format!("model.layers.{layer}.self_attn.q_proj")),
        )?
        .forward(&n_c)?;
        let q_g = candle_nn::linear_no_bias(
            cfg.dim,
            cfg.dim,
            vb_g.pp(format!("model.layers.{layer}.self_attn.q_proj")),
        )?
        .forward(&n_g)?;
        let k_c = candle_nn::linear_no_bias(
            cfg.dim,
            cfg.dim,
            vb_c.pp(format!("model.layers.{layer}.self_attn.k_proj")),
        )?
        .forward(&n_c)?;
        let k_g = candle_nn::linear_no_bias(
            cfg.dim,
            cfg.dim,
            vb_g.pp(format!("model.layers.{layer}.self_attn.k_proj")),
        )?
        .forward(&n_g)?;
        let v_c = candle_nn::linear_no_bias(
            cfg.dim,
            cfg.dim,
            vb_c.pp(format!("model.layers.{layer}.self_attn.v_proj")),
        )?
        .forward(&n_c)?;
        let v_g = candle_nn::linear_no_bias(
            cfg.dim,
            cfg.dim,
            vb_g.pp(format!("model.layers.{layer}.self_attn.v_proj")),
        )?
        .forward(&n_g)?;
        ...
        let mask_c =
            Tensor::from_slice(&m, (1, 1, seq_len, seq_len), &cpu)?.broadcast_as(att_c.shape())?;
        ...
        let o_c = candle_nn::linear_no_bias(
            cfg.dim,
            cfg.dim,
            vb_c.pp(format!("model.layers.{layer}.self_attn.o_proj")),
        )?
        .forward(&y_c)?;
        let o_g = candle_nn::linear_no_bias(
            cfg.dim,
            cfg.dim,
            vb_g.pp(format!("model.layers.{layer}.self_attn.o_proj")),
        )?
        .forward(&y_g)?;
        x_c = (&o_c + &residual_c)?;
        x_g = (&o_g + &residual_g)?;
        ...
        let rms2_c = candle_nn::rms_norm(
            cfg.dim,
            cfg.norm_eps,
            vb_c.pp(format!("model.layers.{layer}.post_attention_layernorm")),
        )?;
        let rms2_g = candle_nn::rms_norm(
            cfg.dim,
            cfg.norm_eps,
            vb_g.pp(format!("model.layers.{layer}.post_attention_layernorm")),
        )?;
        ...
        let g_c = candle_nn::linear_no_bias(
            cfg.dim,
            cfg.hidden_dim,
            vb_c.pp(format!("model.layers.{layer}.mlp.gate_proj")),
        )?
        .forward(&n2_c)?;
        let g_g = candle_nn::linear_no_bias(
            cfg.dim,
            cfg.hidden_dim,
            vb_g.pp(format!("model.layers.{layer}.mlp.gate_proj")),
        )?
        .forward(&n2_g)?;
        let u_c = candle_nn::linear_no_bias(
            cfg.dim,
            cfg.hidden_dim,
            vb_c.pp(format!("model.layers.{layer}.mlp.up_proj")),
        )?
        .forward(&n2_c)?;
        let u_g = candle_nn::linear_no_bias(
            cfg.dim,
            cfg.hidden_dim,
            vb_g.pp(format!("model.layers.{layer}.mlp.up_proj")),
        )?
        .forward(&n2_g)?;
        ...
        let d_c = candle_nn::linear_no_bias(
            cfg.hidden_dim,
            cfg.dim,
            vb_c.pp(format!("model.layers.{layer}.mlp.down_proj")),
        )?
        .forward(&m_c)?;
        let d_g = candle_nn::linear_no_bias(
            cfg.hidden_dim,
            cfg.dim,
            vb_g.pp(format!("model.layers.{layer}.mlp.down_proj")),
        )?
        .forward(&m_g)?;
        x_c = (&d_c + &residual_c)?;
        x_g = (&d_g + &residual_g)?;
    ...
    let logits_c =
        candle_nn::linear_no_bias(cfg.dim, cfg.vocab_size, vb_c.pp("lm_head"))?.forward(&x_c)?;
    let logits_g =
        candle_nn::linear_no_bias(cfg.dim, cfg.vocab_size, vb_g.pp("lm_head"))?.forward(&x_g)?;
