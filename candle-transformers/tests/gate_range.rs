...
fn stats(label: &str, t: &Tensor) -> Result<()> {
    ...
    println!("{label}: range=[{mn}, {mx}] nan={nnan} inf={ninf} n={}", v.len());
    Ok(())
}
...
fn gate_range() -> Result<()> {
    ...
    println!("silu worst i={wi} d={worst} cpu={} gpu={} gate={}", sc[wi], sg[wi], gc[wi]);

    let mul_c = (&s_c * &u_c)?;
    let mul_g = (&s_g * &u_g)?;
    println!("mul md {}", maxdiff(&mul_c, &mul_g)?);
    let mc = mul_c.flatten_all()?.to_vec1::<f32>()?;
    let mg = mul_g.to_device(&Device::Cpu)?.flatten_all()?.to_vec1::<f32>()?;
    let uc = u_c.flatten_all()?.to_vec1::<f32>()?;
    worst=0.0; wi=0;
    for (i,(a,b)) in mc.iter().zip(mg.iter()).enumerate() {
        let d=(a-b).abs(); if d>worst {worst=d; wi=i;}
    }
    println!("mul worst i={wi} d={worst} cpu={} gpu={} silu_c={} silu_g={} up_c={} up_g={} gate={}",
        mc[wi], mg[wi], sc[wi], sg[wi], uc[wi],
        u_g.to_device(&Device::Cpu)?.flatten_all()?.to_vec1::<f32>()?[wi], gc[wi]);
    Ok(())
}
