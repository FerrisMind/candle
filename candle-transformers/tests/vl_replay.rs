...
fn rotate_half(xs: &Tensor) -> Result<Tensor> {
    ...
    let xs2 = xs
        .narrow(candle::D::Minus1, last / 2, last - last / 2)?
        .contiguous()?;
...
fn replay() -> Result<()> {
    ...
    let _cos = st.iter().find(|(n, _)| n == "rope_cos").unwrap().1.clone();
    ...
    let qkv_v = hs_v
        ...
        .permute((1, 0, 2, 3))?
        .contiguous()?;
    let qc = qkv_c.i(0)?.squeeze(0)?.contiguous()?.to_dtype(DType::F32)?;
    let kc = qkv_c.i(1)?.squeeze(0)?.contiguous()?.to_dtype(DType::F32)?;
    let vc = qkv_c.i(2)?.squeeze(0)?.contiguous()?.to_dtype(DType::F32)?;
    let qv = qkv_v.i(0)?.squeeze(0)?.contiguous()?.to_dtype(DType::F32)?;
    let kv = qkv_v.i(1)?.squeeze(0)?.contiguous()?.to_dtype(DType::F32)?;
    let vv = qkv_v.i(2)?.squeeze(0)?.contiguous()?.to_dtype(DType::F32)?;
