...
impl Storage {
    ...
    pub(crate) fn same_device(&self, rhs: &Self, op: &'static str) -> Result<()> {
        ...
        let same_device =
            if self.device().is_metal() || self.device().is_wgpu() || self.device().is_vulkan() {
                // On stateful non-CUDA GPU backends, require the device handle to be exactly the same
                // rather than only having the same physical location.
    ...
    pub(crate) fn inplace_op<const N: usize, C: InplaceOpN<N>>(
        ...
        match self {
            ...
            Storage::Wgpu(dst) => inplace_dispatch!(dst, Wgpu, wgpu_fwd, wgpu_fwd_aliased),
            Storage::Vulkan(dst) => inplace_dispatch!(dst, Vulkan, vulkan_fwd, vulkan_fwd_aliased),
        }
    }
