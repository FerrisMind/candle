//! Regression test for the vulkan last-dim reduce unit-stride bug (P6 / MobileSAM).
...
//! memory instead of strided elements -> silently wrong result on vulkan (CPU correct).
//! The fix routes such views to `run_reduce_non_last_dim`, which materializes via a
//! strides-aware copy. This test compares the vulkan result against the CPU reference.
#![cfg(feature = "vulkan")]

use candle_core::{DType, Device, Result, Tensor};

fn vulkan_device() -> Result<Device> {
    Device::new_vulkan(0)
}

#[test]
fn vulkan_noncontig_last_dim_sum_matches_cpu() -> Result<()> {
    let device = match vulkan_device() {
        Ok(d) => d,
        Err(e) => {
            eprintln!("vulkan unavailable: {e}; skipping");
            return Ok(());
        }
    };
    ...
    let vals: Vec<f32> = (0..b * c * l)
        ...
        .collect();
    // Contiguous [B, C, L] on vulkan, then transpose to [B, L, C]: last-dim stride = L.
...
#[test]
fn vulkan_noncontig_last_dim_argmax_matches_contiguous() -> Result<()> {
    let device = match vulkan_device() {
        Ok(d) => d,
        Err(e) => {
            eprintln!("vulkan unavailable: {e}; skipping");
